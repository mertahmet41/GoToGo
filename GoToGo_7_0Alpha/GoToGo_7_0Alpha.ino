...
#define motor2Pin 3
#define gazpot 0
#define solmotorgaz 0
#define sagmotorgaz 0
#define EDSButon 0
#define direksiyonpot




//--------------------EDS SİSTEM AYARLARI ------------
const float direksiyonaci = 90;
#define maxdireksiyonaci -
int dengekatsayisi = 20;   //örnek bir değer
...
gazseviyesi = analogRead(potgaz);
//gazseviyesi = map(analogRead(potgaz), 0, 1023, 0, 100);

//------------------------------------------------------------------------------------// Pot Gaz Yazdırma Elektronik Diferensiyel Sistemi

if(analogRead(EDSButon) == 1) {
  if(EDS == 1){
    EDS = 0;
  }else if(EDS == 0){
    //EDS = 1;                                              //Daha Test Edilmediğinden Aktif değil
  }
}

if(EDS == 1){
uygulama = ((|direksiyonaci|/maxdireksiyonaci) * dengekatsayisi);
if(direksiyonaci > 20){

analogWrite(solmotorgaz,gazseviyesi + uygulama);
analogWrite(sagmotorgaz,gazseviyesi - uygulama);
}else if(direksiyonaci < -20){
analogWrite(solmotorgaz,gazseviyesi - uygulama);
analogWrite(sagmotorgaz,gazseviyesi + uygulama);
}else{
analogWrite(solmotorgaz,gazseviyesi);
analogWrite(sagmotorgaz,gazseviyesi);
}
